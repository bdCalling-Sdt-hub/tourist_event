        ...
        {packages?.data?.packages &&
          packages?.data?.packages?.map((item: any) => (
            <div
              style={{
                background:
                  packages?.data?.userplan == item?._id
                    ? "var(--color-blue-300)"
                    : "var(--color-blue-200)",
              }}
              className="w-full h-full between-center gap-4 flex-col p-4 py-8 card-shadow rounded-md"
              key={item?.name}
            >
              <div className="text-center">
                ...
                <p className="start-center gap-2 my-3">
                  <FaCheck /> Business Name & Image{" "}
                </p>

                {item?.contactInfoLocation && (
                  <p className="start-center gap-2 my-3">
                    <FaCheck /> Contact Info & Location{" "}
                  </p>
                )}
                ...
                {Number(item?.eventsOrSpecials) > 1 && (
                  <p className="start-center gap-2 my-3">
                    <FaCheck />{" "}
                    {Number(item?.eventsOrSpecials) >= 5000
                      ? "Unlimited Events"
                      : `${item?.eventsOrSpecials} Events/Specials`}{" "}
                  </p>
                )}

                {Number(item?.featuredEvents) > 1 && (
                  <p className="start-center gap-2 my-3">
                    <FaCheck /> {item?.featuredEvents} Featured Events{" "}
                  </p>
                )}
              </div>
              <button
