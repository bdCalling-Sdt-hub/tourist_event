...
const Banner = ({ location, date, end_date, event_image, name, time, social_media }: Props) => {
    return (
        <div className='h-[400px] relative md:h-[600px] w-full rounded-md overflow-hidden mt-5'>
            ...
            <div style={{
                ...
                }} className='text flex justify-start items-center gap-2'><FaLocationDot /> {location}</p>
                <p className='start-center bg-[var(--color-blue-500)] text-[var(--color-white)] w-fit p-1 px-4 mt-2 rounded-md gap-2'>
                    <FaCalendar /> {date?.split('T')?.[0]} at {time} to {end_date?.split('T')?.[0]}
                </p>
