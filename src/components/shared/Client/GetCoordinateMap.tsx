...
const GetCoordinateMap = ({
  ...
  useEffect(() => {
    const map = L.map("map").setView(
      [
        coordinates?.lat?.toFixed(5) || 9.61541,
        coordinates?.lng?.toFixed(5) || 275.3713,
      ],
      13,
    );
    setMapInstance(map);
    ...
    if (coordinates) {
      L.marker([coordinates.lat.toFixed(5), coordinates.lng.toFixed(5)]).addTo(
        map,
      );
    }
    //
  ...
  return (
    <div>
      ...
      {coordinates && (
        <p className="p-2">
          <strong>Clicked Coordinates:</strong> Latitude:{" "}
          {coordinates.lat.toFixed(5)}, Longitude: {coordinates.lng.toFixed(5)}
          <br />
